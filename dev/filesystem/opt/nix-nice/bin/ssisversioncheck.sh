...
diff=$(
	(
		xsltproc <(echo "$pkgXSLT") *.dtsx   | sed 's/^/pkg\t/';
		xsltproc <(echo "$prjXSLT") *.dtproj | sed 's/^/prj\t/'
	) | trdsql -icsv -id '\t' -oh "
...
echo "$diff" | trdsql -ih -oat 'select * from -';
